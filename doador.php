      ...
      <tbody>
        ...
        if ($resultado != null) {
          while ($row = $resultado->fetch_assoc()) {
            ...
            echo '<td style="width: 10%"><a href="excluirDoacao.php?id_doacao=' . $row['id_doacao'] . '&data_doacao=' . $row['data_doacao'] . '&ds_hemocentro=' . $row['ds_hemocentro'] . '&ds_observacao=' . $row['ds_observacao'] . '"class="button button-excluir">Excluir</a></td>';
            echo '</tr>';
          }
        }
        $conexao->close();
